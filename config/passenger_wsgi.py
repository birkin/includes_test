...
- passenger env-vars loading under python3.x is enabled via the `SenEnv` entry in the project's httpd/passenger.conf section.
  - usage: `SetEnv INC_TEST__SETTINGS_PATH /path/to/django_template__env_settings.sh`
...
For activating the virtual-environment manually, don't source the settings file directly. Instead, add to `project_env/bin/activate`:
  export INC_TEST__SETTINGS_PATH="/path/to/django_template__env_settings.sh"
  source $INC_TEST__SETTINGS_PATH
...
ENV_SETTINGS_FILE = os.environ['INC_TEST__SETTINGS_PATH']  # set in `httpd/passenger.conf`, and `env/bin/activate`
